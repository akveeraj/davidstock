...
# anyone on any server to use your FormMail script on their web site.        #
	...
	@referers = ('davidostick.co.uk','www.davidostick.co.uk');
...
#
# Any email addresses must match *exaclty*; any domains (ie, no '@' sign)
...
sub check_url {
    ...
    if ($check_referer != 1) { print "Status: 404 Not found\n\n";exit 0 }
}
...
sub fill_recipients {
    ...
    foreach $domain (@domains) {
				if ($domain=~/\@/) {
					my ($namepart, $domainpart)=split('\@',$domain);
					$domainpart=~s/([\.\-])/\\$1/g;
					$domainpart=~s/(\w)/\[\u$1\l$1\]/g;
					push @return_recips, "\Q$namepart\E\@$domainpart";
