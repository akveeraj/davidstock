...
--------------------
  ...
  @recipients = &fill_recipients(@referers);
    ...
    and email addresses, it turns them into regular expressions. For domains,
    these regular expressions will only allow e-mail messages to go to a
    ...
    Any full email addresses will match (only) themselves.
    ...
    Essentially, it only allows A-Z, a-z, 0-9, - and . in the local address
    ...
    @recipients = &fill_recipients('yourself@yourdomain.com');

    Now the only valid recipient is that one e-mail address.

    If there are several, simply do:

    @recipients = &fill_recipients('user1@yourdomain.com','user2@their.domain.com');
